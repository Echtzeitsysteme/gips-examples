...
public class EmoflonGtAppUtils {
	...
	public static void extractFiles(final String workspacePath) {
		...
		final File target = new File(workspacePath + IhtcvirtualpreprocessingAPI.patternPath);
		if (target.exists()) {
			return;
		}
		try (final InputStream is = IhtcvirtualpreprocessingAPI.class
				.getResourceAsStream("/ihtcvirtualpreprocessing/api/ibex-patterns.xmi")) {
