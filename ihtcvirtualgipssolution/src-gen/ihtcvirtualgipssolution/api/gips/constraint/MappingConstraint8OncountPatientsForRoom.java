...
public class MappingConstraint8OncountPatientsForRoom extends GipsMappingConstraint<IhtcvirtualgipssolutionGipsAPI, CountPatientsForRoomMapping>{
	public MappingConstraint8OncountPatientsForRoom(final IhtcvirtualgipssolutionGipsAPI engine, final MappingConstraint constraint) {
	...
	protected List<Term> buildVariableLhs(final CountPatientsForRoomMapping context) {
		...
		terms.add(new Term(engine.getNonMappingVariable(context, "patientCount"), builder_3(context)));
		return terms;
	}
	...
	protected void builder_1(final List<Term> terms, final CountPatientsForRoomMapping context) {
		engine.getMapper("selectedShiftToRoster").getMappings().values().parallelStream()
					.map(mapping -> (SelectedShiftToRosterMapping) mapping)
		.filter(elt -> elt.getVsr().getShift().getRoom().equals(context.getR()) && elt.getVsr().getShift().getShiftNo() == context.getS().getShiftNo() + 1)
		.forEach(elt -> {
	...
	protected void builder_2(final List<Term> terms, final CountPatientsForRoomMapping context) {
		engine.getMapper("selectedShiftToRoster").getMappings().values().parallelStream()
					.map(mapping -> (SelectedShiftToRosterMapping) mapping)
	...
	protected double builder_3(final CountPatientsForRoomMapping context) {
		return (-1.0) * ((1.0) * (1000));
	}
}
