...
public class RuleConstraint18OnselectExtendingShiftToFirstWorkload extends GipsRuleConstraint<IhtcvirtualgipssolutionGipsAPI, SelectExtendingShiftToFirstWorkloadMatch, SelectExtendingShiftToFirstWorkloadRule>{
	public RuleConstraint18OnselectExtendingShiftToFirstWorkload(final IhtcvirtualgipssolutionGipsAPI engine, final RuleConstraint constraint, final SelectExtendingShiftToFirstWorkloadRule rule) {
...
//		});
	...
	protected void builder_2(final List<Term> terms, final SelectExtendingShiftToFirstWorkloadMatch context) {
		...
		indexer.getMappingsOfNode(context.getNextvsw()).parallelStream()
				.map(mapping -> (SelectedExtendingShiftToFirstWorkloadMapping) mapping)
				.forEach(elt -> {
					terms.add(new Term(elt, (double)((2) * (-1.0)) * (1.0)));
				});
