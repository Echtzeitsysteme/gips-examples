...
public class MappingConstraint28OnotsPerSurgeon extends GipsMappingConstraint<IhtcvirtualgipssolutionGipsAPI, OtsPerSurgeonMapping>{
	public MappingConstraint28OnotsPerSurgeon(final IhtcvirtualgipssolutionGipsAPI engine, final MappingConstraint constraint) {
	...
	protected List<Term> buildVariableLhs(final OtsPerSurgeonMapping context) {
		...
		terms.add(new Term(engine.getNonMappingVariable(context, "penalizedOTs"), 1.0));
		terms.add(new Term(engine.getNonMappingVariable(context, "otCount"), builder_0(context)));
		return terms;
	}
	...
	protected double builder_0(final OtsPerSurgeonMapping context) {
		return (-1.0) * (1.0);
	}
}
