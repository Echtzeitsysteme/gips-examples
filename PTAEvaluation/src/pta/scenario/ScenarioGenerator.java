...
public class ScenarioGenerator extends PTAModelGenerator {
	...
	public PersonTaskAssignmentModel generate() {
		...
		for (int noP = 1; noP <= numOfProjects; noP++) {
			int start = (superimposed || previousProject == null) ? 1
					: previousProject.getInitialWeekNumber() + previousProject.getWeeksUntilLoss();
			...
			for (int noT = 1; noT <= numOfTasks; noT++) {
				Task currentTask = (previousTask == null) ? addTask(currentProject.getName(), "Task_" + noT)
						: addTask(currentProject.getName(), "Task_" + noT, previousTask.getName());
...
//					}
		...
		for (Project p : projects.values()) {
			...
			for (Task t : p.getTasks()) {
				for (Requirement r : t.getRequirements()) {
					...
					List<Person> persons = skillType2level2person.get(r.getSkillType().getName())
							.get(r.getSkillLevel());
	...
	public Person getOrSpawnPerson(final String skillType, int skillLevel) {
		...
		if (level2person == null) {
			...
			if (rnd.nextDouble() <= personSpawnRate) {
				person = addPerson("Person_" + (this.persons.size() + 1), salary(skillLevel), flexibility(skillLevel),
						overtimeSalary(skillLevel), new HashMap<>());
			} else {
				List<Person> candidates = this.persons.values().stream().filter(p -> p.getSalary() == salary)
						.collect(Collectors.toList());
				// Case 1.2: No persons exist, spawn a new person
				if (candidates.isEmpty()) {
					person = addPerson("Person_" + (this.persons.size() + 1), salary(skillLevel),
							flexibility(skillLevel), overtimeSalary(skillLevel), new HashMap<>());
				} else { // Case 1.3: Persons exist, pick one at random if salary is suitable, else
							// create a new person.
					person = candidates.get(rnd.nextInt(candidates.size()));
				}
			}

			// Add skill with the required type anyways, since it didn't exist in the first
			// place.
		...
		if (persons == null) {
			...
			if (rnd.nextDouble() <= personSpawnRate) {
				...
				// Find persons that match the salary and do not have a skill with the required
				...
				if (candidates.isEmpty()) {
					person = addPerson("Person_" + (this.persons.size() + 1), salary(skillLevel),
							flexibility(skillLevel), overtimeSalary(skillLevel), new HashMap<>());
				} else { // Case 2.2: A person with the requirement as described above exists -> pick one
							// at random.
					person = candidates.get(rnd.nextInt(candidates.size()));
		...
		if (rnd.nextDouble() <= personSpawnRate) {
			Person person = addPerson("Person_" + (this.persons.size() + 1), salary(skillLevel),
					flexibility(skillLevel), overtimeSalary(skillLevel), new HashMap<>());
			addSkill(person.getName(), skillLevel, skillType);
			persons.add(person);
			return person;
		} else { // Case 3.2: A suitable person exists -> return the person if the salary
					// matches, else create a new person.
			List<Person> candidates = persons.stream().filter(p -> p.getSalary() == salary)
					.collect(Collectors.toList());
			if (candidates.isEmpty()) {
				return addPerson("Person_" + (this.persons.size() + 1), salary(skillLevel), flexibility(skillLevel),
						overtimeSalary(skillLevel), new HashMap<>());
			} else {
	...
	public void createSkillTypes() {
		double avgReqs = expectedValue(nProjects) * expectedValue(tasksPerProject) * expectedValue(reqPerTask)
				* skillTypeSpawnRate;
		double upper = (nProjects.getUpper() - 1) * (tasksPerProject.getUpper() - 1) * (reqPerTask.getUpper() - 1)
				* skillTypeSpawnRate;
