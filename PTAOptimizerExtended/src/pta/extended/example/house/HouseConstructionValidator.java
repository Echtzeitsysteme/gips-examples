	...
	public boolean validateProjects() {
		...
		for(Project p : model.getProjects()) {
			...
			valid &= p.getTasks().stream()
					...
					.distinct()
					.filter(week -> week.getNumber() < p.getInitialWeekNumber() || week.getNumber() < p.getStart().getNumber())
					.findAny().isEmpty();
			
			valid &= p.getTasks().stream()
					.flatMap(t -> t.getWeeks().stream())
					.distinct()
					.filter(week -> week.getNumber() > p.getInitialWeekNumber() + p.getWeeksUntilLoss() || week.getNumber() > p.getStart().getNumber() + p.getWeeksUntilLoss())
					.findAny().isEmpty();
			
			List<Week> weeks = p.getTasks().stream().flatMap(t -> t.getWeeks().stream()).distinct().collect(Collectors.toList());
...
//				}).reduce(0.0, (sum, cost) -> sum + cost);
			...
			if(totalCost <= p.getReward()) {
				...
				System.out.println("INFO: Project planning exceeds budget. Total salary is: " + totalCost + "€, while the planned spending limit was: " + p.getReward() + "€.");
			}
	...
	public boolean checkAssignmentsTask(final Task task) {
		...
		if(task.getPrevious() != null && !task.getPrevious().isEmpty()) {
			List<Week> previousWeeks = task.getPrevious().stream().flatMap(t -> t.getWeeks().stream()).distinct().collect(Collectors.toList());
...
//			System.out.println(valid);
	...
	public boolean checkRequirement(final Requirement requirement) {
		...
		valid &= !requirement.getOffers().stream()
				.map(offer -> (Person) offer.eContainer())
				.filter(person -> person.getSkills().stream()
						.filter(skill -> skill.getName().equals(requirement.getName()) && skill.getLevel() >= requirement.getSkillLevel()).findAny().isEmpty()).findAny().isPresent();
	...
	public boolean validatePersons() {
		...
		for(Person person : model.getPersons()) {
			Set<Week> offeredWeeks = person.getOffers().stream().map(offer -> offer.getWeek()).collect(Collectors.toSet());
			for(Week week : model.getWeeks()) {
				valid &= offeredWeeks.contains(week);
			}
			valid &= person.getOffers().stream().filter(offer -> offer.getRequirements().size() > 1).findAny().isEmpty();
		}
		return valid;
	}
