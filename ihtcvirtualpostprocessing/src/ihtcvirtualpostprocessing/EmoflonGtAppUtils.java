...
public class EmoflonGtAppUtils {
	...
	public static void extractFiles(final String workspacePath) {
		...
		final File target = new File(workspacePath + IhtcvirtualpostprocessingAPI.patternPath);
		if (target.exists()) {
			return;
		}
		try (final InputStream is = IhtcvirtualpostprocessingAPI.class
				.getResourceAsStream("/ihtcvirtualpostprocessing/api/ibex-patterns.xmi")) {
