...
/**
 * Yen path finding algorithm that is used to generate the shortest K paths for
 * all models. Uses the base Dijkstra implementation. Heavily based on this
...
public class Yen implements IPathGen {
	...
	/**
	 * Implementation of the path finding algorithm by Yen (see reference above).
	 * Finds the K shortest paths from a source to a given target node in a given
	 * substrate network.
	 *
	...
	private List<List<SubstrateNode>> yen(final SubstrateNetwork net, final SubstrateNode source,
		...
		for (int k = 1; k < K; k++) {
			...
			for (int i = 0; i < A.get(k - 1).size() - 1; i++) {
				...
				final Set<SubstrateNode> ignoredNodes = new HashSet<>();
				final Set<SubstrateLink> ignoredLinks = new HashSet<>();
				...
				for (final List<SubstrateNode> p : A) {
					if (p.size() - 1 > i && rootPath.equals(p.subList(0, i + 1))) {
						...
						// same
						// root path.
						final Link toIgnore = ModelFacade.getInstance().getLinkFromSourceToTarget(p.get(i),
								p.get(i + 1));
				...
				// Remove first node in spurPath, because it is the same as the spurNode and it
				// is also
				// contained in rootPath.
				spurPath.remove(0);

				// Entire path is made up of the root path and spur path.
				final List<SubstrateNode> totalPath = new LinkedList<>();
				...
				// The sets for ignored nodes and ignored links will implicit be restored by the
				// loop.
			}
			...
			if (B.isEmpty()) {
				...
				// or there are no spur paths at all - such as when both the source and sink
				// vertices
	...
	/**
	 * Finds the best candidate in the list of lists (equals the one with the lowest
	 * amount of nodes) and pops it.
	 *
	...
	private List<SubstrateNode> popBestCandidate(List<List<SubstrateNode>> candidates) {
		// Sort candidates by length and then by IDs to ensure same order for every
		// permutation of
		// the input list.
		sortCandidates(candidates);

		// After sorting as described above, the search can be replaced by popping the
		// first one.
	...
	private void sortCandidates(List<List<SubstrateNode>> candidates) {
		Collections.sort(candidates, new Comparator<List<SubstrateNode>>() {
			/**
			 * Sorts to given lists of substrate nodes according to: (1) Smaller list size
			 * first. (2) Alphabetical order of node names.
			 */
			@Override
	...
	/**
	 * Calculates and returns the first K fastest paths from a given start node in a
	 * given network. (More specific: It calculates the first K paths with the
	 * smallest amount of hops from the start node to all other nodes.) This method
	 * returns a map of all substrate nodes mapped to a list of of lists of
	 * substrate links from start node to the key of the map.
	 *
	 ...
	 * @return Map of SubstrateNodes to lists of lists of SubstrateLinks that form
	...
	public Map<SubstrateNode, List<List<SubstrateLink>>> getAllKFastestPaths(final SubstrateNetwork net,
		...
		final Map<SubstrateNode, List<List<SubstrateLink>>> paths = new HashMap<>();
	...
	public Map<SubstrateNode, List<SubstrateLink>> getAllFastestPaths(final SubstrateNetwork net,
		...
		final Map<SubstrateNode, List<SubstrateLink>> output = new HashMap<>();

		final Map<SubstrateNode, List<List<SubstrateLink>>> kOutput = getAllKFastestPaths(net, start, 1);
	...
	/**
	 * Translates a list of lists of substrate nodes to a list of lists of substrate
	 * links connecting the nodes together. This one is a wrapper that calls
	 * {@link #translate(List)} for every item in the outer list given.
	 *
	 * @param input List of lists of substrate nodes to convert.
	 * @return List of list of substrate links that form the path of all given
	 *         substrate nodes.
	 */
	private List<List<SubstrateLink>> translateAll(final List<List<SubstrateNode>> input) {
		final List<List<SubstrateLink>> output = new LinkedList<>();
	...
	/**
	 * Translates a list of substrate nodes to a list of substrate links connecting
	 * the nodes together.
	 *
	 * @param nodes List of substrate nodes to convert.
	 * @return List of substrate links that form the path of all given substrate
	 *         nodes.
	 */
	private List<SubstrateLink> translate(final List<SubstrateNode> nodes) {
		...
		for (int i = 0; i < nodes.size() - 1; i++) {
			final Link l = ModelFacade.getInstance().getLinkFromSourceToTarget(nodes.get(i), nodes.get(i + 1));
