...
public class EmoflonGtAppUtils {
	...
	public static void extractFiles(final String workspacePath) {
		final File target = new File(workspacePath + VirtualrulesAPI.patternPath);
		if (target.exists()) {
			return;
		}
		try (final InputStream is = VirtualrulesAPI.class
				.getResourceAsStream("/ihtcgips/virtualrules/api/ibex-patterns.xmi")) {
